"""
COL774 Assignment 1 - Part 2(c), VERSION 2: given columns + feature selection.

    python3 part_c.py dataset_dir/ model.pkl final_features.csv

Version 1 with one addition: the 392 supplied columns are ranked and cut down
to the best SELECT_DIMS of them before the logistic layer.

Why selection helps here specifically. The metric M = TPR - 100*FPR punishes a
false positive as much as 100/N of recall, and the fastest way to manufacture
false positives is to fit noise: with 392 columns and only a few thousand
training rows the model has plenty of room to latch onto columns that separate
the training patients by accident. Columns are therefore scored by mean
out-of-fold |AUC - 0.5| across *patient-grouped* folds, so a column that only
works on a handful of patients cannot outrank one that generalises.

Version 3 adds raw-signal ECG features, version 4 adds a spline basis, and
version 5 picks among all of them automatically.
...
def auc_score(y_true, score):
    ...
    if not finite.all():
        score = np.where(finite, score,
                         np.nanmedian(score[finite]) if finite.any() else 0.0)
...
def main():
    ...
    ranking = rank_features(X_train, y_train, groups)
    keep = np.sort(np.argsort(-ranking)[:min(SELECT_DIMS, X_train.shape[1])])
    phi_train, phi_test = X_train[:, keep], X_test[:, keep]
    if phi_train.shape[1] >= 500:
        raise RuntimeError(f"phi has {phi_train.shape[1]} dims, spec needs < 500")

    top = [feature_cols[j] for j in np.argsort(-ranking)[:5]]
    c, class_weight, cv_m = tune_hyperparameters(phi_train, y_train, groups)
    print(f"[v2 selected] d={phi_train.shape[1]}/{X_train.shape[1]} C={c} "
    ...
    if val_df is not None and "label" in val_df.columns:
        phi_val = apply_preprocessor(
            val_df[feature_cols].to_numpy(dtype=np.float64), prep)[:, keep]
